...
export default async function Page() {
  ...
  try {
    ...
    const { data: rows } = await supabase.from("intents").select("*").is("deleted_at", null).order("updated_at", { ascending: false });
  ...
  const columns = [
    ...
    { key: "default_priority", label: "우선순위" },
    { key: "status", label: "상태" },
  ...
  return <DataTable title="Intents" description="PUCO가 해당 상황에서 달성하려는 UX 목적" data={data} columns={columns} newHref="/studio/intents/new" />;
}
