...
export default async function Page() {
  ...
  try {
    ...
    const { data: rows } = await supabase.from("context_states").select("*").is("deleted_at", null).order("updated_at", { ascending: false });
  ...
  return <DataTable title="Context States" description="센서값과 시나리오에서 파악한 현재 상태" data={data} columns={columns} newHref="/studio/contexts/new" />;
}
